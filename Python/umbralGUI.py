...
imagenL = ImageTk.PhotoImage(tools.greyScale(imagen1))
#Primera imagen
viewImagen1 = Label(root,image=imagenL).place(x=0,y=0)
#Segunda imagen
imagenL2 = ImageTk.PhotoImage(tools.simpleUmbralizacion(imagen1,0))
viewImagen2 = Label(root,image=imagenL2)
viewImagen2.place(x=501,y=0)
#Actualizar la imagen del label
def show(val):
    imagenL3 = ImageTk.PhotoImage(tools.simpleUmbralizacion(imagen1,int(val)))
    viewImagen2.configure(image=imagenL3)
    viewImagen2['image']=imagenL3
    viewImagen2.image=imagenL3
#Deslizador
w = Scale(root,from_= 0,to_= 255,orient=HORIZONTAL,length=1000,tickinterval=10,command=show)
w.place(x=0,y=525)
#Guardar la imagen
def guardar():
        imagenL3 = tools.simpleUmbralizacion(imagen1,w.get())
